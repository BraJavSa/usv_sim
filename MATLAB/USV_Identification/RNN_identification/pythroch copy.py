...
class USVDataset(torch.utils.data.Dataset):
    def __init__(self, T_u, T_r, pass_vel_u, pass_vel_v, pass_vel_r, vel_u, vel_v, vel_r):
    ...
    def __getitem__(self, idx):
        return self.T_u[idx], self.T_r[idx], self.pass_vel_u[idx], self.pass_vel_v[idx], self.pass_vel_r[idx], self.vel_u[idx], self.vel_v[idx], self.vel_r[idx]
...
def create_dataset(data):
    ...
    dataset = USVDataset(T_u, T_r, pass_vel_u, pass_vel_v, pass_vel_r, vel_u, vel_v, vel_r)
    return dataset, T_u, T_r, pass_vel_u, pass_vel_v, pass_vel_r, vel_u, vel_v, vel_r

# Load data
data = loadmat('/home/javipc/catkin_ws/src/usv_sim/MATLAB/USV_Identification/Sindy_Identification/ident_usv_2.mat')
data_val = loadmat('/home/javipc/catkin_ws/src/usv_sim/MATLAB/USV_Identification/Sindy_Identification/muestreo_externo_2.mat')

train_dataset, T_u, T_r, pass_vel_u, pass_vel_v, pass_vel_r, vel_u, vel_v, vel_r = create_dataset(data)
val_dataset, T_u_val, T_r_val, pass_vel_u_val, pass_vel_v_val, pass_vel_r_val, vel_u_val, vel_v_val, vel_r_val = create_dataset(data_val)

# Hyperparameters
input_size = 5  # Input features: T_u, T_r, pass_vel_u, pass_vel_v, pass_vel_r
hidden_size = 200
...
for epoch in range(num_epochs):
    for i, (T_u, T_r, pass_vel_u, pass_vel_v, pass_vel_r, vel_u, vel_v, vel_r) in enumerate(train_dataloader):
        # Prepare input and target tensors
        input_tensor = torch.stack([T_u, T_r, pass_vel_u, pass_vel_v, pass_vel_r], dim=1).unsqueeze(1)
...
with torch.no_grad():
    for T_u_val, T_r_val, pass_vel_u_val, pass_vel_v_val, pass_vel_r_val, vel_u_val, vel_v_val, vel_r_val in val_dataloader:
        # Prepare input tensor
        input_tensor_val = torch.stack([T_u_val, T_r_val, pass_vel_u_val, pass_vel_v_val, pass_vel_r_val], dim=1).unsqueeze(1)
...
plt.subplot(3, 1, 1)
plt.plot(range(len(vel_u_val_all)), vel_u_val_all, label='Velocidad de Surgencia Real')
plt.plot(range(len(vel_u_pred_all)), vel_u_pred_all, label='Velocidad de Surgencia Predicha')
plt.xlabel('Índice de Tiempo')
plt.ylabel('Velocidad de Surgencia (m/s)')
plt.legend()

plt.subplot(3, 1, 2)
plt.plot(range(len(vel_v_val_all)), vel_v_val_all, label='Velocidad de Balanceo Real')
plt.plot(range(len(vel_v_pred_all)), vel_v_pred_all, label='Velocidad de Balanceo Predicha')
plt.xlabel('Índice de Tiempo')
